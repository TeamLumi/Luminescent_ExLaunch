...
static void showIncomingRequestDialog() {
    ...
    // Load translatable format template (e.g. "%s with %s?")
    const char* promptFmt = getMPMessageCStr("SS_mp_RequestPrompt", "%s with %s?");

    static char promptBuf[128];
    snprintf(promptBuf, sizeof(promptBuf), promptFmt, actionTextBuf, nameBuf);
...
static void showTradePreview() {
    ...
    *reinterpret_cast<int32_t*>(reinterpret_cast<uintptr_t>(uiZukanReg) + 0x98) = 1;
    ...
    if (nameStr != nullptr && !System::String::IsNullOrEmpty(nameStr)) {
        ...
        const char* promptFmt = getMPMessageCStr("SS_mp_TradeConfirm",
            "Trade for %s's Lv.%u%s %s%s?");
        snprintf(s_tradeConfirmMsgBuf, sizeof(s_tradeConfirmMsgBuf), promptFmt,
                 partnerName, level, genderStr, cname.c_str(),
                 isRare ? " \xe2\x98\x85" : "");
    } else {
        snprintf(s_tradeConfirmMsgBuf, sizeof(s_tradeConfirmMsgBuf),
                 "Trade for %s's pokemon?", partnerName);
    }
        ...
        if (party != nullptr) {
            ...
            if (partnerPoke != nullptr) {
                ...
                if (nameStr != nullptr && !System::String::IsNullOrEmpty(nameStr)) {
                    ...
                    const char* promptFmt = getMPMessageCStr("SS_mp_TradeConfirm",
                        "Trade for %s's Lv.%u%s %s%s?");
                    snprintf(msgBuf, sizeof(msgBuf), promptFmt,
                             partnerName, level, genderStr, cname.c_str(),
                             isRare ? " \xe2\x98\x85" : "");
                }
            }
