...
{
	float x = std::clamp<float>((t - x0) / (x1 - x0), 0.0f, 1.0f);
...
float FCEasing::EaseStepped(float t, int Steps)
{
	...
	{
		return std::floor(Steps * t) / Steps;
	}
}
...
float FCEasing::EaseInOutExpo(float t)
{
	...
	if (t < 0.5f)
	{
		return std::pow(2, 10 * (2 * t - 1) - 1);
	}
	else
	{
		return 1 - std::pow(2, -10 * (2 * t - 1) - 1);
	}
}
...
float FCEasing::EaseInOutCirc(float t)
{
	...
	if (t2 < 1)
	{
		return (1 - std::sqrt(1 - t2 * t2)) * .5f;
	}
	else
	{
		float m = t - 1;
		return (std::sqrt(1 - 4 * m * m) + 1) * .5f;
	}
}
...
float FCEasing::EaseInElastic(float t, float Amplitude, float Period)
{
	...
	else
	{
		...
		if (Amplitude > 1)
		{
			s = Period * std::asin(1.0f / Amplitude) / (2.0f * PI);
		}

		return -(Amplitude * std::pow(2, 10 * m) * std::sin((m - s) * (2.0f * PI) / Period));
	}
}
...
// {
// 	float m = t - 1;
// 	return -std::pow(2, 10 * m) * std::sin((m * 40 - 3) * PI / 6);
// }
...
float FCEasing::EaseOutElastic(float t, float Amplitude, float Period)
{
	...
	else
	{
		...
		return 1.0f + Amplitude * std::pow(2, -10 * t) * std::sin((t - s) * (2.0f * PI) / Period);
	}
}
// baked-in-parameters version
// float FCTween::EaseOutElastic(float t)
// {
// 	return 1 + std::pow(2, 10 * (-t)) * std::sin((-t * 40 - 3) * PI / 6);
// }
float FCEasing::EaseInOutElastic(float t, float Amplitude, float Period)
{
	...
	else
	{
		...
		if (m < 0)
		{
			return .5f * -(Amplitude * std::pow(2, 10 * m) * std::sin((m - s) * (2.0f * PI) / Period));
		}
		else
		{
			return 1.0f + .5f * (Amplitude * std::pow(2, -10 * t) * std::sin((t - s) * (2.0f * PI) / Period));
		}
	}
}
// baked-in-parameters version
...
// 	if (s < 0)
// 	{
// 		return -.5f * std::pow(2, 10 * s) * std::sin(k);
// 	}
// 	else
// 	{
// 		return 1 + .5f * std::pow(2, -10 * s) * std::sin(k);
// 	}
// }
