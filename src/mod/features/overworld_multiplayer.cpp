...
// Active battle partner tracking — persists through the battle scene even
// after the interaction state machine resets.  Checked by onPlayerLeave
// to detect mid-battle disconnects.
static int32_t s_activeBattlePartner = -1;
static bool s_battlePartnerDisconnected = false;
static bool s_disconnectSignaled = false;  // one-shot: BSP/NetClient flags set
...
int32_t overworldMPGetActiveBattlePartner() {
    return s_activeBattlePartner;
}

bool overworldMPIsBattlePartnerDisconnected() {
    return s_battlePartnerDisconnected;
}
...
static void onOverworldMPReceivePacket(void* pr, void* /*method*/) {
    ...
    case OWMP_DATA_ID_TEAMUP_BATTLE_ACK: {
        ...
        case 0: { // HEADER
            ...
            // Reset trainer party accumulation buffer
            memset(tu.trainerPartyBuf, 0, sizeof(tu.trainerPartyBuf));
            ...
            if (!isAck) {
                ...
                memset(tu.partnerTrainerBuf, 0, sizeof(tu.partnerTrainerBuf));
                tu.partnerTrainerBufSize = 0;
                tu.partnerTrainerCount = 0;
                tu.partnerTrainerValid = false;
            }
        ...
        case 2: { // TRAINER_POKE (trainer party Pokemon — from initiator or from ACK)
            ...
            // Choose buffer based on packet type (BATTLE vs ACK)
            uint8_t* destBuf = s_tuAccumIsAck ? tu.partnerTrainerBuf : tu.trainerPartyBuf;
            int32_t destBufSize = s_tuAccumIsAck
            ...
            if (s_tuAccumIsAck) {
                ...
                tu.partnerTrainerCount = s_tuAccumTrainerCount;
            } else {
                tu.trainerPartyBufSize = s_tuAccumTrainerCount * POKE_FULL_DATA_SIZE;
    ...
    // -----------------------------------------------------------------------
    // 0xCB: Team-up sync wait (trainer rendezvous)
    // -----------------------------------------------------------------------
    case OWMP_DATA_ID_TEAMUP_SYNC_WAIT: {
        int32_t fromStation = il2cpp_vcall_int(pr, PR_FROM_STATION);
        if (fromStation < 0 || fromStation >= OW_MP_MAX_PLAYERS) return;
        ...
        int32_t effectID = -1;
        il2cpp_vcall_read_out(pr, PR_READ_S32_OUT, &effectID);
        int32_t randomTeamMode = -1;
        il2cpp_vcall_read_out(pr, PR_READ_S32_OUT, &randomTeamMode);
        int32_t zoneID = -1;
    ...
    // -----------------------------------------------------------------------
    // 0xCC: Team-up sync cancel (going solo)
    // -----------------------------------------------------------------------
    case OWMP_DATA_ID_TEAMUP_SYNC_CANCEL: {
        int32_t fromStation = il2cpp_vcall_int(pr, PR_FROM_STATION);
        if (fromStation < 0 || fromStation >= OW_MP_MAX_PLAYERS) return;
    ...
    case OWMP_DATA_ID_CUSTOM_COLORS: {
        ...
        auto& remote = s_mpContext.remotePlayers[fromStation];
        // Don't check isActive — 0xCD can arrive before the join event due to
        // PIA mesh event ordering. Data is preserved through join's Clear().
...
void overworldMPUpdate(float deltaTime) {
    ...
    // Check for Y-button interaction with nearby remote players
    if (s_mpContext.state == OverworldMPState::Connected) {
    ...
    // Deferred EncountStart — waits for ACK before allowing battle transition
    overworldMPTickDeferredEncount();
    ...
    for (int i = 0; i < OW_MP_MAX_PLAYERS; i++) {
        ...
        if (remote.colorRefreshTimer > 0.0f) {
            ...
            if (remote.colorRefreshTimer <= 0.0f) {
                ...
                // Use the pointer captured during OnEnable at Instantiate time
                auto* cvComp = (ColorVariation::Object*)remote.colorVariationComp;
                if (cvComp != nullptr) {
                    extern void UpdateColorVariation(ColorVariation::Object* variation);
                    if (remote.colorId == -1 && remote.hasCustomColors) {
                        cvComp->fields.ColorIndex = 0;
...
void overworldMPOnPlayerJoin(int32_t stationIndex) {
    ...
    // Preserve custom colors that may have arrived before the join event
    // (PIA mesh sends 0xCD before the session event fires on the receiver)
    bool savedHasCustomColors = remote.hasCustomColors;
    float savedFieldColors[18], savedBattleColors[18];
    if (savedHasCustomColors) {
        memcpy(savedFieldColors, remote.customFieldColors, sizeof(savedFieldColors));
        memcpy(savedBattleColors, remote.customBattleColors, sizeof(savedBattleColors));
    }
    ...
    // Restore pre-arrived custom colors
    if (savedHasCustomColors) {
        remote.hasCustomColors = true;
        memcpy(remote.customFieldColors, savedFieldColors, sizeof(savedFieldColors));
        memcpy(remote.customBattleColors, savedBattleColors, sizeof(savedBattleColors));
    }
...
void overworldMPOnPlayerLeave(int32_t stationIndex) {
    ...
    // Abort any in-progress interaction with this station (trade, battle, etc.)
    // Must be called BEFORE despawn so the interact handler can still reference state.
    overworldMPOnInteractionPartnerLeft(stationIndex);
...
static void onCharacterAssetLoaded(Il2CppObject* loadedAsset, MethodInfo* /*method*/) {
    ...
    owmpSetCaptureStation(stationIndex);
    ...
    // during Instantiate. The TypeInfo/MethodInfo at 0x04C57F80/0x04c703e0 are
    // permanently null, so GetComponent cannot work. The OnEnable hook fires
    // synchronously during Instantiate and captures the component pointer.
    {
        extern ColorVariation::Object* owmpGetCapturedColorVariation(int32_t);
        extern void owmpClearCapturedColorVariation(int32_t);
        auto* cvComp = owmpGetCapturedColorVariation(stationIndex);
        owmpClearCapturedColorVariation(stationIndex);
        if (cvComp != nullptr) {
            remote.colorVariationComp = cvComp;
            extern void UpdateColorVariation(ColorVariation::Object* variation);
            if (remote.colorId == -1 && remote.hasCustomColors) {
                cvComp->fields.ColorIndex = 0;
                UpdateColorVariation(cvComp);
...
// Net.Client.Update @ 0x203A460
// During team-up battles, actively clears non-fatal error bits BEFORE and
// AFTER Orig. This is the primary error suppression mechanism:
//   - Before Orig: prevents the battle engine from seeing errors set by PIA
//     callbacks between frames (OnReceivePacketEx→0x400, sendToClientCore→0x200)
//   - After Orig: catches errors set during Update's own processing
// This breaks the error signal propagation chain: if Player B's errors are
// cleared before sendToClientCore runs, no error signal is sent to Player A.
static constexpr uint32_t BTLNET_NONFATAL_ERRORS = 0x080 | 0x100 | 0x200 | 0x400;

HOOK_DEFINE_TRAMPOLINE(NetClient$$Update) {
    static void Callback(void* __this) {
        // Diagnostic: unconditional log to verify the hook fires at all.
        // The previous build showed NO Update logs — this tells us definitively.
        ...
        // The previous isInBattleScene() check was too strict — the flag is
        // set AFTER EncountStart releases, but BtlNet starts earlier.
        bool isTeamUp = overworldMPIsTeamedUp();

        // PRE-ORIG: clear non-fatal errors before the +0x60==0 check in Orig.
        // This is the CRITICAL fix — prevents PIA callback errors (0x80 etc.)
        // from permanently stopping the Update loop.
        // Skip clearing if partner disconnected — let errors propagate to end battle.
        if (isTeamUp && !s_battlePartnerDisconnected) {
            uint32_t pre = *(uint32_t*)((uintptr_t)__this + 0x60);
            uint32_t nonfatal = pre & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
                *(uint32_t*)((uintptr_t)__this + 0x60) = pre & ~BTLNET_NONFATAL_ERRORS;
        ...
        int32_t errorBefore = *(int32_t*)((uintptr_t)__this + 0x60);
        Orig(__this);
        int32_t errorAfter = *(int32_t*)((uintptr_t)__this + 0x60);

        // POST-ORIG: clear non-fatal errors set during Update's processing
        if (isTeamUp && !s_battlePartnerDisconnected) {
            uint32_t nonfatal = (uint32_t)errorAfter & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
                *(uint32_t*)((uintptr_t)__this + 0x60) = (uint32_t)errorAfter & ~BTLNET_NONFATAL_ERRORS;
...
HOOK_DEFINE_TRAMPOLINE(NetClient$$sendToServerVersionCoreAll) {
    static void Callback(void* __this) {
        // Use simpler condition — same fix as Update hook
        // Skip clearing if partner disconnected — let errors propagate.
        if (overworldMPIsTeamedUp() && !s_battlePartnerDisconnected) {
            uint32_t errors = *(uint32_t*)((uintptr_t)__this + 0x60);
            uint32_t nonfatal = errors & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
                *(uint32_t*)((uintptr_t)__this + 0x60) = errors & ~BTLNET_NONFATAL_ERRORS;
        ...
        // Also clear AFTER Orig — sendToClientCore (called from here) may set
        // 0x200 on PIA send failure to AI slots
        if (isOverworldMPActive() && overworldMPIsInBattleScene() && overworldMPIsTeamedUp() && !s_battlePartnerDisconnected) {
            uint32_t errors = *(uint32_t*)((uintptr_t)__this + 0x60);
            uint32_t nonfatal = errors & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
                *(uint32_t*)((uintptr_t)__this + 0x60) = errors & ~BTLNET_NONFATAL_ERRORS;
...
HOOK_DEFINE_TRAMPOLINE(BattleProc$$UpdateInitialze) {
    static void Callback(void* __this) {
        ...
        // If partner disconnected during init, signal the battle to terminate
        // (same logic as UpdateMainRun — set BSP flags + Net.Client fatal error).
        bool partnerGone = s_battlePartnerDisconnected;
        if (partnerGone && isBattle) {
            if (!s_disconnectSignaled) {
                ...
                if (bspClass != nullptr && bspClass->static_fields != nullptr) {
                    auto* bsp = bspClass->static_fields->setupParam;
                    if (bsp != nullptr) {
                ...
                if (netClient != nullptr) {
                    ...
                    *(uint32_t*)((uintptr_t)netClient + 0x60) = errors | 0x01;
                    *(uint8_t*)((uintptr_t)netClient + 0xa1) = 1;
                }
            }
            // Don't clear errors — let them propagate to break the init loop
            Orig(__this);
            return;
        }

        // PRE-ORIG: clear non-fatal BtlNet errors from netClient.
        // Net.Client.Update at 0x203A460 does NOT fire (confirmed by missing
        // FIRST CALL log), so the error clearing in that hook is dead code.
        // BattleProc.UpdateInitialze fires unconditionally each frame, making
        // it the reliable place to suppress errors before the battle engine
        // processes them.
        if (overworldMPIsTeamedUp() && !partnerGone && netClient != nullptr) {
            uint32_t errors = *(uint32_t*)((uintptr_t)netClient + 0x60);
            uint32_t nonfatal = errors & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
                *(uint32_t*)((uintptr_t)netClient + 0x60) = errors & ~BTLNET_NONFATAL_ERRORS;
        ...
        // POST-ORIG: clear errors set during this frame's Orig processing
        if (overworldMPIsTeamedUp() && !partnerGone && netClient != nullptr) {
            uint32_t errors = *(uint32_t*)((uintptr_t)netClient + 0x60);
            uint32_t nonfatal = errors & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
...
HOOK_DEFINE_TRAMPOLINE(BattleProc$$UpdateMainRun) {
    static void Callback(void* __this) {
        ...
        // If the battle partner disconnected, signal the battle engine to terminate.
        // Set isDisconnectOccur + commError on BSP so storeBattleResult sees an
        // abnormal end, and inject a fatal error into Net.Client so the battle
        // state machine breaks out of its send/receive loop.
        if (partnerGone && isOverworldMPActive() && overworldMPIsInBattleScene()) {
            if (!s_disconnectSignaled) {
                ...
                // 2. Set fatal error on Net.Client (+0x60) — use bit 0x01 which
                //    is NOT in BTLNET_NONFATAL_ERRORS and will be seen as fatal.
            ...
            // Skip error clearing — let all errors propagate so the battle ends
            Orig(__this);
            return;
        }

        // PRE-ORIG: clear non-fatal BtlNet errors during main battle loop.
        // Same rationale as UpdateInitialze — Net.Client.Update doesn't fire,
        // so we clear here instead.
        if (isTeamUp && !partnerGone && s_cachedBattleNetClient != nullptr) {
            uint32_t errors = *(uint32_t*)((uintptr_t)s_cachedBattleNetClient + 0x60);
            uint32_t nonfatal = errors & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
                *(uint32_t*)((uintptr_t)s_cachedBattleNetClient + 0x60) = errors & ~BTLNET_NONFATAL_ERRORS;
        ...
        // POST-ORIG: catch errors set during this frame
        if (isTeamUp && !partnerGone && s_cachedBattleNetClient != nullptr) {
            uint32_t errors = *(uint32_t*)((uintptr_t)s_cachedBattleNetClient + 0x60);
            uint32_t nonfatal = errors & BTLNET_NONFATAL_ERRORS;
            if (nonfatal) {
...
extern void exl_overworld_mp_interact_hooks();

void exl_overworld_multiplayer_main() {
    s_mpContext.Initialize();

    // Install hooks from overworld_mp_interact.cpp (trade preview)
    exl_overworld_mp_interact_hooks();
    ...
    // Net.Client Update — track error state changes during battle
    NetClient$$Update::InstallAtOffset(0x203A460);
