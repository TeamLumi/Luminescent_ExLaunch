...
const uint32_t SILPH_SCOPE_SOUND_ID = 0x8e8a8de1;

void UseSilphScopeMkII(int32_t itemId, bool fromBag, Dpr::UI::UIBag::__c__DisplayClass127_1::Object* bagDisplayClass)
{
    bool flag = PlayerWork::GetBool((int32_t)FlagWork_Flag::FLAG_SYMBOL_ENCOUNTERS);
    flag = !flag;
    PlayerWork::SetBool((int32_t)FlagWork_Flag::FLAG_SYMBOL_ENCOUNTERS, flag);

    Logger::log("[SilphScope] Toggled symbol encounters: %s\n", flag ? "ON" : "OFF");
    ...
    if (flag)
    {
        ...
        audioManager->PlaySe(SILPH_SCOPE_SOUND_ID, nullptr);
    }
    ...
    if (fromBag)
    {
        ...
        if (flag)
        {
            labelName = System::String::Create("SS_bag_373");
        }
        else
        {
    ...
    else
    {
        ...
        System::String::Object * labelName = System::String::Create("ev_keyitem_silphscopemk2");
