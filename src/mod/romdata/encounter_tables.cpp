...
{
    // Load meta.json to get slot count
    ...
    if (metaJson != nullptr && !metaJson.is_discarded())
    {
        ...
        if (meta.slotCount > 0)
        {
            ...
            int32_t slotIndex = UnityEngine::Random::Range(0, meta.slotCount);
            ...
            if (slotJson != nullptr && !slotJson.is_discarded())
            {
                ...
                Logger::log("GetEncounterTableSlot: Table %d, Slot %d/%d, MonsNo %d, FormNo %d\n",
                    tableID, slotIndex, meta.slotCount, slot.monsNo, slot.formNo);
                return slot;
            }
            else
            {
    ...
    {
        Logger::log("GetEncounterTableSlot: Error parsing meta for table %d!\n", tableID);
    }
